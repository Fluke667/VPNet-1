...
vpnet::init_host_id() {
  local id
  id=$(ip addr show docker0 | grep ether | awk '{print $2}' | awk -F: '{print $5$6}')
...
vpnet::init_network() {
  ...
  # XXX does there always be `docker0` in docker ???
  echo "Setting network filter ..."
  iptables -t nat -A POSTROUTING -s 10.0.0.0/8      -o docker0 -j MASQUERADE || echo "WARN: iptables fail"
  iptables -t nat -A POSTROUTING -s 172.16.0.0/12   -o docker0 -j MASQUERADE || echo "WARN: iptables fail"
  iptables -t nat -A POSTROUTING -s 192.168.0.0/16  -o docker0 -j MASQUERADE || echo "WARN: iptables fail"

  # ip6tables -t nat -A POSTROUTING -s 2a00:1450:400c:c05::/64 -o docker0 -j MASQUERADE
